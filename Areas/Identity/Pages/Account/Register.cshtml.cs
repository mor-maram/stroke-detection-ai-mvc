...
namespace MediAI.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    await _logService.LogAsync(user.Id, "User_Registered", user.Id, HttpContext.GetUserIP());
                    _logger.LogInformation("User created a new account with password.");

                    var userId = await _userManager.GetUserIdAsync(user);
                    await _userManager.AddToRoleAsync(user, UserType.Patient.ToString());
        ...
        private User CreateUser()
        {
            try
            {
                var user = Activator.CreateInstance<Patient>();
                user.FirstName = Request.Form["Input.FirstName"];
                user.LastName = Request.Form["Input.LastName"];                
                user.DateOfBirth = Convert.ToDateTime(Request.Form["Input.DateOfBirth"]);
                return user;
            }
            catch
            {
